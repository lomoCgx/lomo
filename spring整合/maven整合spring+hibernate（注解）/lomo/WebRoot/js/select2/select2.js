define('select2', ['jquery', 'css!lib/select2/select2'], function($) {
	(function() {
		if (window.define) {
		...
		define('select2/results', [
			...
			Results.prototype.displayMessage = function(params) {/*没有查询到数据时*/
				this.clear();
				this.hideLoading();
			...
			Results.prototype.bind = function(container, $container) {
				...
				this.$results.on('mouseup', '.select2-results__option[aria-selected]',
					function(evt) {
						$('li.select2-search').hide().closest('ul').removeClass('pb39px');
						var $this = $(this);
		...
		define('select2/selection/multiple', [
			...
			MultipleSelection.prototype.bind = function(container, $container) {
				...
				this.$selection.on('click', function(evt) {/*【显示/隐藏】目的地下拉输入框*/
					self.trigger('toggle', {
						originalEvent: evt
					});
				});
			...
			MultipleSelection.prototype.selectionContainer = function() {
				var $container = $(
					'<li class="select2-selection__choice ui ' + Bestone.util.getRandomColor() + ' lable">' +
					'<span class="select2-selection__choice__remove" role="presentation">' +
					//'&times;' +
					'<i class="remove large red sign icon"></i>' +
					'</span>' +
					'</li>'
				);
		...
		define('select2/selection/search', [
			...
			Search.prototype.render = function(decorated) {
				var $search = $(
					...
					' role="textbox" />' +
					'</li><span class="mddText">目的地</span>'
				);
		...
		define('select2/data/select', [
			...
			SelectAdapter.prototype.select = function(data) {//---------------------------------------select
				var self = this;

				// If data.element is a DOM nose, use it instead
				if ($(data.element).is('option')) {
					//self.$element.data('arr').push();
					var arr = self.$element.data('arr');

					arr.push($(data.element).val());

					self.$element.data('arr',arr);
					$('.mddText').html('<i class="expand large  icon add fl" style="margin-top:2px;"></i>');

					data.element.selected = true;

					self.$element.val($(data.element).val());
			...
			SelectAdapter.prototype.unselect = function(data) {//---------------------------------------unselect
				var self = this;
				...
				if ($(data.element).is('option')) {
					...
					//删除对应的目的地ID
					var arr = self.$element.data('arr');
					arr.splice($.inArray($(data.element).val(),arr),1);
					if(arr.length<=0){
						$('.mddText').html('目的地');
					}
					self.$element.data('arr',arr);
		...
		define('select2/core', [
			...
			Select2.prototype.open = function() {
				...
				$('.select2-search').show().closest('ul').addClass('pb39px');
				this.trigger('query', {});
			...
			Select2.prototype.close = function() {
				...
				$('.select2-search').hide().closest('ul').removeClass('pb39px');
				this.trigger('close');
			};
